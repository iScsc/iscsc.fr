...
const { DB_USER, DB_PASSWORD, DB_NAME, BACKEND_PORT, CLIENT_ORIGIN } = process.env
...
mongoose
  ...
  .then(() => {
    httpServer.listen(BACKEND_PORT || 3001, () => {
      console.log(`Server listening: http://localhost:${BACKEND_PORT}`)
    })
  })
  .catch(err => {
    console.log(err)
